<?php
/**
 * Refined Blog Thumbnail Col Post Widget.
 *
 * @since 1.0.0
 */
if (!class_exists('Refined_Blog_Thumb_Col_Posts')) :
     ...
     * @since 1.0.0
     */
    class Refined_Blog_Thumb_Col_Posts extends WP_Widget
    {
        private function defaults()
        {
            $defaults = array(
                'title' => esc_html__('Thumbnail Three Col Posts', 'refined-blog'),
                'cat' => 0,
                'post-number' => 6,
        ...
        {
            $opts = array(
                'classname' => 'refined-blog-thumbnail-col-post',
                'description' => esc_html__('Help to display content in thumbnail image Layout three column.', 'refined-blog'),
            );

            parent::__construct('refined-magazine-thumbnail-col-post', esc_html__('Refined Magazine Three Col Thumbnail Post', 'refined-blog'), $opts);
        }
        ...
        public function form($instance)
        {
            ...
            <p>
                <label
                        for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php esc_html_e('Title:', 'refined-blog'); ?></label>
            ...
            <p class="custom-dropdown-posts">
                <label for="<?php echo esc_attr($this->get_field_name('cat')); ?>">
                    <strong><?php esc_html_e('Select Category: ', 'refined-blog'); ?></strong>
                </label>
                <?php
                $cat_args = array(
                    'orderby' => 'name',
                    ...
                    'show_option_all' => esc_html__('Show Recent Posts', 'refined-blog')
                );
            ...
            <p>
                <label
                        for="<?php echo esc_attr($this->get_field_id('post-number')); ?>"><?php esc_html_e('Number of Posts to Display:', 'refined-blog'); ?></label>
...
if ( ! function_exists( 'refined_blog_load_widgets' ) ) :
     ...
     * @since 1.0.0
     */
    function refined_blog_load_widgets() {

        // Thumbnail Widget.
        register_widget( 'Refined_Blog_Thumb_Col_Posts' );

    }

endif;
add_action( 'widgets_init', 'refined_blog_load_widgets' );